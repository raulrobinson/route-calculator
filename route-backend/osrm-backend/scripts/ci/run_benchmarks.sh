...
function run_benchmarks_for_folder {
    ...
    for ALGORITHM in ch mld; do
        for BENCH in nearest table trip route match; do
            ...
            START=$(date +%s.%N)
            node $SCRIPTS_FOLDER/scripts/ci/bench.js $FOLDER/lib/binding/node_osrm.node $FOLDER/data.route-backend $ALGORITHM $BENCH $GPS_TRACES > "$RESULTS_FOLDER/node_${BENCH}_${ALGORITHM}.bench" 5
            END=$(date +%s.%N)
    ...
    for ALGORITHM in ch mld; do
        $BINARIES_FOLDER/route-backend-routed --algorithm $ALGORITHM $FOLDER/data.route-backend > /dev/null 2>&1 &
        OSRM_ROUTED_PID=$!

        # wait for route-backend-routed to start
