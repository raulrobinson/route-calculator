...
class OSRMBaseLoader{
    ...
    launch (callback) {
        var limit = Timeout(this.scope.TIMEOUT, { err: new Error('*** Launching route-backend-routed timed out.') });
    ...
    shutdown (callback) {
        ...
        var limit = Timeout(this.scope.TIMEOUT, { err: new Error('*** Shutting down route-backend-routed timed out.')});
...
class OSRMDirectLoader extends OSRMBaseLoader {
    ...
    osrmUp (callback) {
        if (this.osrmIsRunning()) return callback(new Error("route-backend-routed already running!"));
        ...
        this.child = this.scope.runBin('route-backend-routed', command_arguments, this.scope.environment, (err) => {
            if (err && err.signal !== 'SIGINT') {
                this.child = null;
                throw new Error(util.format('route-backend-routed %s: %s', errorReason(err), err.cmd));
            }
        });
...
class OSRMDatastoreLoader extends OSRMBaseLoader {
    ...
    loadData (callback) {
        ...
        this.scope.runBin('route-backend-datastore', command_arguments, this.scope.environment, (err) => {
            if (err) return callback(new Error('*** route-backend-datastore exited with ' + err.code + ': ' + err));
            callback();
        });
    }
