...
module.exports = function () {
    this.initializeEnv = (callback) => {
        ...
        this.OSRM_EXTRACT_PATH = path.resolve(util.format('%s/%s%s', this.BIN_PATH, 'route-backend-extract', this.EXE));
        this.OSRM_CONTRACT_PATH = path.resolve(util.format('%s/%s%s', this.BIN_PATH, 'route-backend-contract', this.EXE));
        this.OSRM_CUSTOMIZE_PATH = path.resolve(util.format('%s/%s%s', this.BIN_PATH, 'route-backend-customize', this.EXE));
        this.OSRM_PARTITION_PATH = path.resolve(util.format('%s/%s%s', this.BIN_PATH, 'route-backend-partition', this.EXE));
        this.OSRM_ROUTED_PATH = path.resolve(util.format('%s/%s%s', this.BIN_PATH, 'route-backend-routed', this.EXE));
    ...
    this.verifyOSRMIsNotRunning = (callback) => {
        tryConnect(this.OSRM_IP, this.OSRM_PORT, (err) => {
            if (!err) return callback(new Error('*** route-backend-routed is already running.'));
            else callback();
        });
    };
