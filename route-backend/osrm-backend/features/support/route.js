...
module.exports = function () {
    ...
    this.requestUrl = (path, callback) => {
        ...
        runRequest(limit((err, res, body) => {
            if (err) {
                if (err.statusCode === 408) return callback(this.RoutedError('*** route-backend-routed did not respond'));
                else if (err.code === 'ECONNREFUSED')
                    return callback(this.RoutedError('*** route-backend-routed is not running'));
            } else
