...
module.exports = function () {
    ...
    this.buildWaysFromTable = (table, callback) => {
        ...
        var buildRow = (row, ri, cb) => {
            ...
            // NOTE: currently route-backend crashes when processing an isolated oneway with just 2 nodes, so we use 4 edges
            // this is related to the fact that a oneway dead-end street doesn't make a lot of sense

            // if we stack ways on different x coordinates, routability tests get messed up, because route-backend might pick a neighboring way if the one test can't be used.
    ...
    this.extractData = (p, callback) => {
        ...
        fs.exists(stamp, (exists) => {
            ...
            this.runBin('route-backend-extract', util.format('%s --profile %s %s', p.extractArgs, p.profileFile, p.inputCacheFile), p.environment, (err) => {
                if (err) {
                    return callback(new Error(util.format('route-backend-extract %s: %s', errorReason(err), err.cmd)));
                }
    ...
    this.contractData = (p, callback) => {
        ...
        fs.exists(stamp, (exists) => {
            ...
            this.runBin('route-backend-contract', util.format('%s %s', p.contractArgs, p.processedCacheFile), p.environment, (err) => {
                if (err) {
                    return callback(new Error(util.format('route-backend-contract %s: %s', errorReason(err), err)));
                }
                fs.writeFile(stamp, 'ok', callback);
            });
        });
    ...
    this.partitionData = (p, callback) => {
        ...
        fs.exists(stamp, (exists) => {
            ...
            this.runBin('route-backend-partition', util.format('%s %s', p.partitionArgs, p.processedCacheFile), p.environment, (err) => {
                if (err) {
                    return callback(new Error(util.format('route-backend-partition %s: %s', errorReason(err), err.cmd)));
                }
                fs.writeFile(stamp, 'ok', callback);
            });
        });
    ...
    this.customizeData = (p, callback) => {
        ...
        fs.exists(stamp, (exists) => {
            ...
            this.runBin('route-backend-customize', util.format('%s %s', p.customizeArgs, p.processedCacheFile), p.environment, (err) => {
                if (err) {
                    return callback(new Error(util.format('route-backend-customize %s: %s', errorReason(err), err)));
                }
                fs.writeFile(stamp, 'ok', callback);
            });
        });
