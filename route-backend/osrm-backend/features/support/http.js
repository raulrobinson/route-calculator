        ...
        var runRequest = (cb) => {
            ...
            request(this.query, (err, res, body) => {
                if (err && err.code === 'ECONNREFUSED') {
                    return cb(new Error('*** route-backend-routed is not running.'));
        ...
        runRequest(limit((err, res, body) => {
            if (err) {
                if (err.statusCode === 408)
                    return callback(new Error('*** route-backend-routed did not respond'));
                else if (err.code === 'ECONNREFUSED')
                    return callback(new Error('*** route-backend-routed is not running'));
            }
